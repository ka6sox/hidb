...
def run_search():
    if request.method == 'POST':
      ...
      query = 'SELECT i.id, model_no, serial_no, description, qty, cost, date_added, sublocation, photo, (SELECT description FROM locations l WHERE location = l.id) as location FROM items i WHERE '
      valid_query = False
